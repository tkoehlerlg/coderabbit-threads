...
- The `cr.label` field describes conversation state with the bot (`bot-pushback` / `bot-acked` / `awaiting-bot` / `untouched` / `outdated-unresolved` / `resolved`). The skill adds its own `triage` label on top (`likely-fixed` / `still-applies` / `contested` / `unclear` / `out-of-scope`). Do not collapse them.
- CodeRabbit acknowledges replies through two channels, not one: a follow-up text comment, or a GitHub emoji reaction (`EYES` → "I'm looking at this", `ROCKET` / `THUMBS_UP` / `HEART` / `HOORAY` → agreed, `THUMBS_DOWN` / `CONFUSED` → pushback). `cr` surfaces both: `cr check` returns `state: bot_reacted` when only a reaction is present, and `cr threads` exposes `last_human_bot_reaction.{content,signal,created_at}` plus per-comment `reactions[]`. The label is reaction-aware (since v0.9.0): a positive reaction flips `awaiting-bot` → `bot-acked`; a disagree reaction flips it to `bot-pushback`; EYES alone stays `awaiting-bot` (the bot has not decided yet).
