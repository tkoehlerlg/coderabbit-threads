...
- **Polling loop no longer hangs on reaction-only acks.** Up to v0.8.0, `cr check` only watched for text replies. When CodeRabbit acknowledged a reply with 🚀 ROCKET (its emoji ack for "shipped / sounds good") and never posted a text follow-up, `cr check` returned `awaiting` indefinitely and Step 7 kept the thread in the poll queue until the 5-minute timeout — then exited without resolving it, even under `RESOLVE_POLICY = auto`. The new reaction-aware `bot_reacted` state closes that gap.
